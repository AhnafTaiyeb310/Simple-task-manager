...
export default function TaskItem({ task, onStatusChange, onDelete, isUpdating, isDeleting }: Props) {
  ...
  return (
    <article className={`group border-b border-white/10 px-5 py-4 transition hover:bg-white/[0.03] ${isBusy ? "opacity-60" : ""}`}>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0 flex-1">
        <div className="mb-2 flex flex-wrap items-center gap-2">
          <span className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${statusStyles[task.status]}`}>
        ...
        <h3 className="break-words text-base font-semibold text-white">{task.title}</h3>
        {task.description && (
          <p className="mt-1 break-words text-sm leading-6 text-zinc-400">{task.description}</p>
        )}
      </div>
