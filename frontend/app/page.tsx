...
export default function HomePage() {
  ...
  const [pendingTasks, setPendingTasks] = useState<Record<number, 'updating' | 'deleting' | null>>({});
  ...
  async function handleAdd(title: string, description: string, status: TaskStatus) {
    try {
            ...
            <div className="space-y-3 px-5 py-5">
              {statusOrder.map((status) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className={`size-2.5 rounded-full ${statusSummary[status].accent}`} />
