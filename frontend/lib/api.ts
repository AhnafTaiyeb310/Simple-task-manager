...
function handleRequestError(err: unknown): never {
  if (err instanceof Error && err.message.includes("Failed to fetch")) {
    throw new Error("Connection refused: Please verify the backend server is running");
  }
  throw err;
}
...
export async function createTask(title: string, description: string, status: TaskStatus): Promise<Task>{
  try {
    const res = await fetch(BASE_API, {
        method: 'POST',
